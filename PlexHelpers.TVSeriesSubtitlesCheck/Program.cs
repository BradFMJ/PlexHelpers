...
namespace PlexHelpers.TVSeriesSubtitlesCheck
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            _tvShows = _tvShows.Where(p => p.HasAllEpisodes).Where(p => !p.HasAllSubtitles).OrderBy(p => p.Episodes.Count).ToList();
            ...
            foreach (var tvShow in _tvShows)
            {
                if (tvShow.HasAllEpisodes && !tvShow.HasAllSubtitles)
                {
                    ...
                    //Update Medusa
                    string requestUri = "subtitleShow?showslug=" + indexString + tvShow.indexer_id;
                    Console.WriteLine("{0}/{1} CHECKING SUBTITLES FOR {2}  | {3}.", count, totalCount, tvShow.show_name, requestUri);
